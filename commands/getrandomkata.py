...
async def get_and_check_languague(l):
    ...
    if language not in languages:
        await message.channel.send(f"{message.author.mention}, {args[0]} is not a language, to see a list of languages use {prefix}languagues")
        return False
    return language
#It check if the level(l) exist and if it does return the level, if it doesn't exist it return False
async def get_and_check_lvl(l):
    kyu_lvls = ["1","2","3","4","5","6","7","8"]
    lvl = l.replace('-','')
    if lvl not in kyu_lvls:
        await message.channel.send(f"{message.author.mention}, {args[1]} is not a valid level!")
        return False
    return lvl
...
async def getrandomkata(message,args, prefix):
    ...
    if args == []:
        ...
        language = await get_and_check_languague(args[0])
        if language == False: return

        kata_data = await getkata(language, None, None, topics)

    elif len(args) == 2:
        language = await get_and_check_languague(args[0])
        if language == False: return
        
        lvl = await get_and_check_lvl(args[1])
        if lvl == False: return

        kata_data = await getkata(language, lvl, None, topics)
        ...
        topic = ""
        args.pop(0) # Quita el lenguaje de la lista de argumentos
        args.pop(0) # Quita el nivel de la lista de argumentos
        for x in args:
    ...
    embed_kata.add_field(name="Category", value=kata_data['category'], inline=False)
